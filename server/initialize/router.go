...
func Routers() *gin.Engine {
	...
	PrivateGroup.Use(middleware.JWTAuth()).Use(middleware.CasbinHandler())
	{
		...
		router.InitNginxPoolRouter(PrivateGroup)             // Pool管理
		router.InitNginxClusterRouter(PrivateGroup)          // NginxCluster管理
		router.InitNginxNodeRouter(PrivateGroup)             // NginxNode管理
		router.InitPoolNodeRouter(PrivateGroup)              // PoolNode管理
		router.InitNginxDomainRouter(PrivateGroup)           // Domain管理
		router.InitDomainRuleRouter(PrivateGroup)            //Domain转发规则管理
		router.InitDomainRuleInsRouter(PrivateGroup)         //规则指令管理
		router.InitPublishHistoryRouter(PrivateGroup)
		router.InitNginxDomainCertRouter(PrivateGroup)
	}
